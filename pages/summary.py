...
has_any      = has_sales or has_labor or has_expenses
...
else:            _missing.append("Toast POS (Sales & Food Cost)")
...
if has_sales and kpi:
    ...
    if has_labor:
        alerts = sum([prime > prime_w, food_pct > food_w, labor > labor_w])
        warnings = sum([
        ...
        # Can only assess food cost without labour
        alerts   = int(food_pct > food_w)
        warnings = int(food_t < food_pct <= food_w)
        if alerts:
            hs_status, hs_label = "alert", "Needs Attention — food cost alert"
        elif warnings:
            hs_status, hs_label = "warning", "Watch Closely — food cost above target"
        else:
            hs_status, hs_label = "good", "Food Cost Healthy (Payroll not connected)"
    ...
    elif not has_labor and (alerts or warnings):
        with st.expander("View issues →", expanded=True):
            (st.error if food_pct > food_w else st.warning)(
                f"**Food Cost %: {food_pct:.1f}%** — "
                f"{'above alert threshold' if food_pct > food_w else 'above target'} "
                f"(target ≤{food_t:.0f}%, alert >{food_w:.0f}%)  \n"
                "Cost of goods as a % of revenue. Review supplier invoices, portion sizes, and wastage logs."
            )
...
if has_sales and kpi:
    ...
    with c4: st.metric("Food Cost %",    threshold_badge(kpi["food_cost_pct"], food_t, food_w))
    with c5:
        if has_labor:
    ...
    with c6: st.metric("Avg. Check",     format_currency(kpi["avg_check"]))
